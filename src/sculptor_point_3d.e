...
feature {NONE} -- Initialization
	...
	make_from_array (a_coords: ARRAY [REAL_64])
...
feature -- Queries
	...
	distance_to (a_other: SCULPTOR_POINT_3D): REAL_64
		...
		local
			l_dist_sq: REAL_64
		do
			l_dist_sq := (x - a_other.x) ^ 2.0 + (y - a_other.y) ^ 2.0 + (z - a_other.z) ^ 2.0
			Result := {DOUBLE_MATH}.sqrt (l_dist_sq)
		ensure
